...
class userController {
  ...
  static async editProfile(req, res) {
    const { email } = req.user;
    const { profileimg, first_name, last_name, phonenumber } = req.body;

    const responseEditProfileValidation = validateEditProfile({
      first_name,
      last_name,
      phonenumber,
    });
    if (responseEditProfileValidation.error)
      return res
        .status(400)
        .json({ Error: `${responseEditProfileValidation.error}` });
  ...
  static async getAppliedTasks(req, res) {
    try {
      const { id } = req.user;
      const getAppliedTasksQuery = `SELECT * FROM task_user WHERE user_id=$1 ORDER BY createdat DESC`;
      const userValue = [id];
      const appliedTasksData = await pool.query(
        getAppliedTasksQuery,
        userValue,
      );
      if (appliedTasksData.rows && appliedTasksData.rows.length > 0) {
        return res.status(200).json({
          status: 'success',
          code: 200,
          data: appliedTasksData.rows,
        });
      }
      return res.status(400).json({
        message: 'failed',
